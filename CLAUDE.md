...
3. **Forwarders** (`npm`/`npx`/`pnpm`/`yarn`/`pip`/`pip3`/`poetry`/`uv`) — registered dynamically from `forward.Managers()`. Each wraps an install, blocks on malware, otherwise execs the real manager. `DisableFlagParsing: true` so every arg reaches the real tool untouched; config comes only from `OSSPREY_API_URL` / `OSSPREY_API_KEY` env vars. `forward.Run` has two modes (`internal/forward/forward.go`): when packages are **named** it checks exactly those (`ParseSpecs` classifies args into `Specs` / `NonPackages` / `ReqFiles`, skipping flag-values, local paths, archives, URLs, VCS refs); when **no** packages are named — a bare `install`/`ci`/`yarn install`/`poetry install`/`uv sync`/`pip install -r` — the manager installs from the project manifest, so it runs a directory scan (`scanProjectFn` → `scan.Run` + `submit.Validate`) and checks every declared dependency rather than falling through unchecked (OSS-1284). Only installs whose sole targets are local/URL refs forward without a check.
   ...
   **`npx` is covered by its own parser** (`internal/forward/npx.go`), not a `verbAt` entry, because it has no verb: `npxInstallAt` matches every invocation and `npxParse` (plugged in through `Manager.parse`) decides what is fetched. Only the first positional can be a package — npx stops reading its own options there and the rest is the program's argv, so `npx cowsay moo` checks `cowsay` and never `moo` (a real npm package that could block). With `--package`/`-p` the positional is a *command* and only the `-p` values are checked. Option arity is **not** hand-listed: npx parses twice (its `bin/npx-cli.js` pre-pass, then npm's nopt), and nopt reads an *unknown* flag as a boolean, so the same flag can mean different things per npm version — `npx --allow-scripts a b` runs `b` on npm 12 (a string option) and `a` on npm 10 (verified against both binaries). `npx_flags.go` is generated from npm 10 and 12's `@npmcli/config` definitions: `npxSwitches` are boolean in every version, `npxOptions` take a value in every version, and anything else — version-dependent or unknown — is ambiguous, so its next token *and* the following positional are both checked. That is the only reading that is safe in both directions, since a hand table is unsafe whichever way it is wrong. `TestNpxTablesAgreeWithInstalledNpm` re-derives the tables from the npm on PATH, so regenerate from the newest npm when it fails. An unpinned name (unless `--prefix`/`-C` redirects the project) that `localBin` finds in the nearest project root's `node_modules` (npm's own localPrefix walk, which stops at the first `package.json`/`node_modules`) is dropped, because npx runs it from there and fetches nothing — without that, every `npx eslint`/`npx tsc` would check a registry package (`tsc` is not typescript) that is never run. Every non-exact spec — unpinned names too — resolves through `registry.ResolveNpmSpec`, which follows npm-pick-manifest step for step: a dist-tag is its exact target; an unpinned name (npa reads it as the range `*`) or a range takes the *default tag* if the range allows it **and it is not deprecated**, else the highest non-deprecated match. `*` is compared by string there, so a prerelease default tag still wins for an unpinned name (`npx --tag beta -p typescript` ran 6.0.0-beta against real npx, latest 7.0.2). `--tag` sets the default tag and `--before`/`--enjoy-by` filter by publish time (full packument, since the abbreviated one has no times; a too-new tag falls back to `<=` its version), both from flags or `npm_config_*` but not `.npmrc`. Never resolve npx specs through plain latest: a malicious release behind `@next`, an old major line, a deprecated latest or a `--before` cut-off would pass on a clean latest. A `--before` we cannot parse skips the affected checks with a warning rather than guessing, since npm's JavaScript `Date` accepts more than we do. An unresolvable specifier (`registry.ErrNoMatch`) fails open with a warning like a registry outage, and is never reported clean. `FetchExec` keeps a no-spec npx (`npx -c`, `--version`, a local bin) away from the manifest-scan path: naming nothing does not mean "install from the manifest" here. No `Lockfile`, so passive npx goes through `passiveAlongside`.

   **Not covered, deliberately (for now):** the other fetch-and-execute verbs — `npm exec`, `pnpm dlx`, `yarn dlx`, `uv tool run` — and `uvx`, which is not shimmed. These name a package but are not install verbs, so they forward unchecked. Do **not** close them by adding `dlx`/`exec` to a `verbAt` list: only the first non-flag token is a package, so `pnpm dlx cowsay moo` would check `moo`. `npxParse` is the shape to reuse (`npm exec` takes the same options). `docs/forwarder.md`'s "What is not checked" section is the user-facing statement of this.
