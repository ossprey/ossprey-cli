...
3. **Forwarders** (`npm`/`npx`/`pnpm`/`yarn`/`pip`/`pip3`/`poetry`/`uv`) — registered dynamically from `forward.Managers()`. Each wraps an install, blocks on malware, otherwise execs the real manager. `DisableFlagParsing: true` so every arg reaches the real tool untouched; config comes only from `OSSPREY_API_URL` / `OSSPREY_API_KEY` env vars. `forward.Run` has two modes (`internal/forward/forward.go`): when packages are **named** it checks exactly those (`ParseSpecs` classifies args into `Specs` / `NonPackages` / `ReqFiles`, skipping flag-values, local paths, archives, URLs, VCS refs); when **no** packages are named — a bare `install`/`ci`/`yarn install`/`poetry install`/`uv sync`/`pip install -r` — the manager installs from the project manifest, so it runs a directory scan (`scanProjectFn` → `scan.Run` + `submit.Validate`) and checks every declared dependency rather than falling through unchecked (OSS-1284). Only installs whose sole targets are local/URL refs forward without a check.
   ...
   **`npx` is covered by its own parser** (`internal/forward/npx.go`), not a `verbAt` entry, because it has no verb: `npxInstallAt` matches every invocation and `npxParse` (plugged in through `Manager.parse`) decides what is fetched. Only the first positional can be a package — npx stops reading its own options there and the rest is the program's argv, so `npx cowsay moo` checks `cowsay` and never `moo` (a real npm package that could block). With `--package`/`-p` the positional is a *command* and only the `-p` values are checked. `npxValueFlags` has the usual asymmetry, and a wrongly listed boolean flag here swallows the package and runs it unchecked; `-s` is left out because legacy npx read it as `--shell` and npm 7+ as `--silent`. An unpinned name that `localBin` finds in the nearest project root's `node_modules` (npm's own localPrefix walk, which stops at the first `package.json`/`node_modules`) is dropped, because npx runs it from there and fetches nothing — without that, every `npx eslint`/`npx tsc` would check a registry package (`tsc` is not typescript) that is never run. Dist-tags and ranges become unpinned and resolve to latest, rather than submitting `latest` as a version. `FetchExec` keeps a no-spec npx (`npx -c`, `--version`, a local bin) away from the manifest-scan path: naming nothing does not mean "install from the manifest" here. No `Lockfile`, so passive npx goes through `passiveAlongside`.
