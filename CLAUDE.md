...
- **Severity** (`internal/severity`) grades a finding `Info < Low < Medium < High < Critical`. A finding below the floor is printed as a `Note:` line and the scan still exits 0 (OSS-1432). The floor is **the account's**, served on the scan response **envelope** as `failing_severity_floor` and set on the SBOM by `submit.Validate` (OSS-1994) -- beside the document on the wire, because an SBOM other formats have to carry is the wrong home for a per-account policy value (the CLI does still write it into its own `-o` output, where it is a diagnostic nothing reads back); `FailingFloor` (`Low`) is only the fallback when a response carries none, which is what keeps an older server behaving exactly as before. `--fail-on <level>` on `scan`/`check` overrides it for one run **in either direction** — most specific wins, not strictest, so do not take the minimum of the two. `--fail-on Info` is how a run fails on everything reported; the older `--fail-on-informational` was removed rather than kept as an alias, since `Info` is the bottom of the scale and said nothing the level does not. Whichever floor wins drives the exit code and the `--report` split together, so the file cannot disagree with the exit status. The forwarders parse no flags of their own (`DisableFlagParsing`), so they take the account's floor with no override. Parsing is deliberately **fail-closed** — an empty or unrecognised *grade* is `Unknown` and `Unknown` fails at every floor, including a raised one; an unreadable *floor* falls back to `FailingFloor` (`severity.ParseFloor`) rather than failing everything. Those two directions are opposite on purpose. Never invert the grade default for convenience; it is the one thing standing between "we could not grade it" and "we passed it".
