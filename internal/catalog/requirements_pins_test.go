...
func TestPinnedRequirements(t *testing.T) {
	cases := []struct {
		...
		// One name pinned twice under mutually exclusive markers is two
		// packages, not one. Keeping only the last let the corrector stamp it
		// onto both, and deduplication then collapsed them into one component.
		name: "every pin for a name is kept, in file order",
		body: "a==1.0.0-beta.1 ; python_version < \"3.9\"\na==2.0.0-beta.1 ; python_version >= \"3.9\"\n",
		want: map[string][]string{"a": {"1.0.0-beta.1", "2.0.0-beta.1"}},
	}}
...
func TestMatchPin(t *testing.T) {
	cases := []struct {
		...
		name:     "the truncated version identifies its pin",
		pins:     []string{"1.0.0-beta.1"},
		reported: "1.0.0",
		want:     "1.0.0-beta.1",
	}, {
		name:     "an exact pin matches itself",
		pins:     []string{"0.1.138.0"},
		reported: "0.1.138.0",
		want:     "0.1.138.0",
	}, {
		// "===2.0.0.post1" reaches us as "=2.0.0.post1".
		name:     "a leading = from arbitrary equality is dropped first",
		pins:     []string{"2.0.0.post1"},
		reported: "=2.0.0.post1",
		want:     "2.0.0.post1",
	}, {
		// Mutually exclusive markers: each syft package must find its own pin.
		name:     "conflicting marker pins are told apart by their prefixes",
		pins:     []string{"1.0.0-beta.1", "2.0.0-beta.1"},
		reported: "1.0.0",
		want:     "1.0.0-beta.1",
	}, {
		name:     "conflicting marker pins, the other side",
		pins:     []string{"1.0.0-beta.1", "2.0.0-beta.1"},
		reported: "2.0.0",
		want:     "2.0.0-beta.1",
	}, {
		// Correcting to either would be a guess, so keep syft's version.
		name:     "two pins truncating alike are ambiguous",
		pins:     []string{"1.0.0-beta.1", "1.0.0-beta.2"},
		reported: "1.0.0",
		want:     "",
	}, {
		name:     "a version no pin explains is left alone",
		pins:     []string{"1.0.0-beta.1"},
		reported: "9.9.9",
		want:     "",
	}, {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, ok := matchPin(tc.pins, tc.reported)
			if tc.want == "" {
				if ok {
					t.Errorf("matchPin(%v, %q) = %q, true; want no match", tc.pins, tc.reported, got)
				}
				return
			}
			if !ok || got != tc.want {
				t.Errorf("matchPin(%v, %q) = %q, %v; want %q, true", tc.pins, tc.reported, got, ok, tc.want)
			}
		})
	}
}
...
// TestCatalog_ConflictingMarkerPinsStayDistinct guards the correction against
// collapsing components. One name pinned to different versions under mutually
// exclusive markers is two packages; stamping one pin onto both let dedup merge
// them, so a version disappeared from the SBOM and went unscanned.
func TestCatalog_ConflictingMarkerPinsStayDistinct(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, dir, "requirements.txt",
		"foo==1.0.0-beta.1 ; python_version < \"3.9\"\n"+
			"foo==2.0.0-beta.1 ; python_version >= \"3.9\"\n")
	...
	want := []string{"1.0.0-beta.1", "2.0.0-beta.1"}
	if !reflect.DeepEqual(versions, want) {
		t.Errorf("foo versions = %v, want %v", versions, want)
	}
}
